          ...
          <div className="mt-10 space-y-10">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0 flex-1 space-y-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-[1fr_160px_auto] sm:items-end">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        className="h-11 border-neutral-200 bg-white text-neutral-900"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-neutral-500">날짜</label>
                      <Input
                        type="date"
                        value={eventDate}
                    ...
                    <Button type="button" variant="outline" className="h-11 border-neutral-300" disabled={pending} onClick={saveHeader}>
                ...
                {!canManageSetlists ? (
                  <div>
                    <h2 className="text-xl font-semibold text-neutral-900">{setlist.title}</h2>
              ...
              {canManageSetlists ? (
                <div className="flex shrink-0 flex-wrap gap-2">
                  <SetlistLineupEditor
                    ...
                    members={teamMembers}
                    triggerClassName="border-neutral-300 text-neutral-800"
                  />
                </div>
              ) : null}
            </div>

            <div className="rounded-2xl border border-neutral-100 bg-neutral-50/40 px-4 py-5 sm:px-6">
              <p className="mb-3 text-xs font-medium uppercase tracking-wider text-neutral-400">라인업</p>
              {setlist.lineup.length === 0 ? (
                <p className="text-sm text-neutral-500">아직 배정된 멤버가 없습니다.</p>
              ) : (
                  ...
                  {groupedLineup
                    ...
                    .map((item) => (
                      <li
                        ...
                        className="rounded-full border border-neutral-200 bg-white px-3 py-1.5 text-sm text-neutral-800"
                      >
                        <span className="text-neutral-400">{teamRoleLabel(item.roleCode)}</span>
            ...
            <div>
              <div className="mb-4 flex items-center justify-between gap-3">
                <h3 className="text-sm font-semibold text-neutral-900">수록곡</h3>
                <span className="text-xs text-neutral-400">{setlist.songs.length}곡</span>
              </div>
            ...
            {canManageSetlists ? (
              <div className="rounded-2xl border border-neutral-200 bg-neutral-50/30 px-4 py-5 sm:px-6">
